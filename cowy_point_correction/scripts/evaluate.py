...
def forward_pass(model: L.LightningModule, dataloader: torch.utils.data.DataLoader):
    ...
    with torch.no_grad():
        for x, y in dataloader:
            ...
            preds.append(y_hat)
            obs.append(y.numpy())
    return np.vstack(preds), np.vstack(obs)
...
def main():
    ...
    pred_train, obs_train = forward_pass(model, dm.train_dataloader())
    pred_val,   obs_val   = forward_pass(model, dm.val_dataloader())
    pred_test,  obs_test  = forward_pass(model, dm.test_dataloader())
